...
def signin(username,password):
	...
	users = c.execute("SELECT pass FROM accounts WHERE user == %s" % (username))
...
def register(username,password):
	db = sqlite3.connect(f)
	c = db.cursor()
	user = c.execute("SELECT user FROM accounts WHERE user == %s" % (username))
	if user:
		return 1
		...
		c.execute("INSERT INTO accounts VALUES (%s,%s)" % (username,password))
		c.close()
		db.close()
		return 6
